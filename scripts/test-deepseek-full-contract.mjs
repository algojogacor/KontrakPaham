...
async function main() {
  ...
  // If DB provider configuration has json format enabled
  if (provider.useJsonResponse) {
    payload.response_format = { type: "json_object" };
  }

  console.log(`Sending full contract prompt to DeepSeek...
- Model: ${provider.model}
- JSON Mode: ${provider.useJsonResponse ? "ENABLED" : "DISABLED"}
- Prompt Chars: System (${SYSTEM_PROMPT.length}) + User (${sampleKosText.length})
`);
